  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      <div className="relative h-36 bg-gray-100">
        <img
          src={property.images?.[0]?.url || "https://placehold.co/400x200?text=No+Image"}
          alt={property.title}
        ...
        <span
          className={`absolute top-2.5 right-2.5 text-[10px] font-bold px-2 py-0.5 rounded-full ${STATUS_CLS[property.status]}`}
        >
          {property.status}
        </span>
      </div>
