...
import {
  ArrowLeft, MapPin, Users, BedDouble, Bath, Bed,
  CheckCircle, Clock, XCircle, Loader2,
  Mail, Phone, Home, ChevronDown
} from "lucide-react";
...
const STATUS = {
  confirmed: { label: "Confirmed", cls: "text-green-600 bg-green-50",  Icon: CheckCircle },
...
const AMENITY_LABELS = {
  wifi: "Wi-Fi", parking: "Parking", pool: "Pool", ac: "AC",
  kitchen: "Kitchen", tv: "TV", heater: "Heater", security: "Security",
};
    ...
    <div className="space-y-5 p-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Booking #{shortId}</h1>
            <p className="text-sm text-gray-400 mt-0.5">Created {fmt(booking.createdAt)}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="p-5">
              <div className="flex items-start justify-between gap-2 mb-1">
                <h2 className="text-base font-bold text-gray-900">{property.title}</h2>
              ...
              {property.amenities?.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-4">
                  {property.amenities.map(a => (
                    <span key={a} className="text-[11px] font-medium bg-gray-50 text-gray-500 px-2.5 py-1 rounded-full border border-gray-100">
                      {AMENITY_LABELS[a] || a}
                    </span>
                  ))}
                </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
              {[
                ...
                { label: "Guests",    value: `${booking.guestsCount} guest${booking.guestsCount !== 1 ? "s" : ""}` },
              ].map(({ label, value }) => (
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400 capitalize">{guest?.role}</p>
              </div>
            </div>
            <div className="space-y-2.5">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className={`px-2 py-0.5 rounded-full font-semibold ${guest?.isActive ? "bg-green-50 text-green-600" : "bg-gray-100 text-gray-400"}`}>
                  {guest?.isActive ? "Active" : "Inactive"}
                </span>
                {guest?.is_verified && (
                  <span className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 font-semibold">Verified</span>
                )}
              </div>
            </div>
          </div>
          ...
          {/* Price breakdown */}
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <h3 className="text-sm font-bold text-gray-900 mb-4">Price Breakdown</h3>
            <div className="space-y-2.5 text-sm">
              <div className="flex justify-between text-gray-500">
                <span>₹{property.pricePerNight?.toLocaleString()} × {n} nights</span>
                <span>₹{(property.pricePerNight * n).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-gray-500">
                <span>Platform fee (10%)</span>
                <span>₹{Math.round(booking.totalPrice * 0.1).toLocaleString()}</span>
              </div>
