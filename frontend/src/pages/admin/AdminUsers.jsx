...
export default function AdminUsers() {
  ...
  useEffect(() => {
    axios
      .get("/admin/getUsers") // adjust endpoint if needed
            ...
            return (
              ...
              >
                ...
                <div className="hidden md:grid grid-cols-[1.5fr_1.5fr_0.6fr_0.6fr_0.8fr_0.5fr] gap-4 px-5 py-3.5 items-center">
                  ...
                  <span
                    className={`text-[11px] font-bold px-2 py-1 rounded-full capitalize w-fit ${ROLE_CLS[u.role] || "bg-gray-100 text-gray-500"}`}
                  >
                    {u.role}
                  </span>
                  <span
                    className={`text-[11px] font-semibold px-2 py-1 rounded-full capitalize w-fit ${STATUS_CLS[status]}`}
                  >
                    {status}
                  </span>
                ...
                <div className="md:hidden flex items-center justify-between px-5 py-4 gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${ROLE_CLS[u.role] || "bg-gray-100 text-gray-500"}`}
                    >
                      {u.role}
                    </span>
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${STATUS_CLS[status]}`}
                    >
                      {status}
                    </span>
                  </div>
